...
class TestFileLists(unittest.TestCase):
	def testOneEntry(self):
		l=fileLists.listTableFiles([
			'2014.0.p.3765.1.1.department.diff.csv',
		])
		self.assertEqual(len(l),1)
		t=l[0]
		self.assertEqual(t.stage,'2014.0.p')
	...
	def testSort(self):
		l=fileLists.listTableFiles([
			...
			'2014.0.p.3765.10.4.department.diff.csv',
			'2014.0.p.3765.1.1.department.diff.csv',
		])
	...
	def testSet(self):
		l=fileLists.listTableFiles([
			'2014.0.p.3765.1.1.department.set(2015,2016).csv',
		])
		self.assertEqual(len(l),1)
